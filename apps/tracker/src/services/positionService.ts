...
const USDSO_DECIMALS = 13; // use 13 of token's 18 decimals → 1 USDso = $100 000
...
function chainProductToProduct(p: ChainProduct): Product {
  const triggerType =
    p.triggerType === 0 ? ("DEPEG" as const) : ("RUG" as const);
  ...
  const utilizationPct =
    poolLimitUsd > 0 ? totalCommittedUsd / poolLimitUsd : 0;
  const healthStatus: Product["healthStatus"] = !p.active
    ? "PAUSED"
    : utilizationPct > 0.8
      ? "WATCH"
      : "HEALTHY";
...
export class PositionService extends EventEmitter<TrackerEvents> {
  ...
  applyChainProduct(p: ChainProduct) {
    ...
    this.db
      .upsertProduct(converted)
      .catch((err) => console.error("[db] upsertProduct failed:", err));
  }
  ...
  getParticipants(): Participant[] {
    ...
    return Array.from(map.entries()).map(([address, posns]) => {
      ...
      return {
        ...
        totalPayoutUsd: claimed.reduce(
          (s, p) => s + (p.claimedPayoutUsd ?? 0),
          0,
        ),
  ...
  getParticipant(address: string): Participant | null {
    ...
    return (
      participants.find((p) => p.address === address.toLowerCase()) ?? null
    );
  }
  ...
  getAnalytics(): AnalyticsSummary {
    ...
    const productStats: ProductStats[] = this.products.map((product) => {
      ...
      return {
        ...
        totalCoverageUsd: posns.reduce((s, p) => s + p.coverageAmountUsd, 0),
        totalPremiumUsd: posns.reduce((s, p) => s + p.premiumUsd, 0),
        totalPayoutsUsd: productClaimed.reduce(
          (s, p) => s + (p.claimedPayoutUsd ?? 0),
          0,
        ),
    ...
    const participants = new Set(
      this.positions.map((p) => p.holder.toLowerCase()),
    ).size;

    return {
      totalActiveCoverageUsd: active.reduce(
        (s, p) => s + p.coverageAmountUsd,
        0,
      ),
      totalLockedPremiumUsd: active.reduce((s, p) => s + p.premiumUsd, 0),
      totalPayoutsUsd: claimed.reduce(
        (s, p) => s + (p.claimedPayoutUsd ?? 0),
        0,
      ),
      ...
      claimRate:
        this.positions.length > 0 ? claimed.length / this.positions.length : 0,
      productStats,
    };
  }
  ...
  updatePosition(next: Position) {
    ...
    this.db
      .upsertPosition(next)
      .catch((err) => console.error("[db] upsertPosition failed:", err));
  ...
  recordLog(entry: AgentLogEntry) {
    ...
    this.db
      .insertLog(entry)
      .catch((err) => console.error("[db] insertLog failed:", err));
    this.emit("log", entry);
  }
