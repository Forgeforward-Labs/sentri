...
import {
  CORE_ADDRESS, VAULT_ADDRESS, USDC_ADDRESS,
  USDC_DECIMALS, ERC20_ABI, INSURANCE_CORE_ABI,
} from '../lib/contracts'
...
function BuyCoverageButton({
  ...
  // 1. Check USDC allowance (vault needs to pull premium from user if needed)
  const { data: allowance } = useReadContract({
    address: USDC_ADDRESS,
    abi: ERC20_ABI,
    functionName: 'allowance',
    args: [address ?? '0x', CORE_ADDRESS ?? '0x'],
    query: { enabled: Boolean(address && USDC_ADDRESS && CORE_ADDRESS) },
  })
  ...
  // 3. Buy position
  const { writeContract: buy, data: buyTxHash, isPending: buying } = useWriteContract()
  const { isLoading: confirming, isSuccess } = useWaitForTransactionReceipt({ hash: buyTxHash })

  const needsApproval = allowance !== undefined && allowance < amountRaw

  function handleClick() {
    if (!CORE_ADDRESS || !USDC_ADDRESS || !VAULT_ADDRESS) return
    ...
    if (needsApproval) {
      approve({
        ...
        args: [CORE_ADDRESS, amountRaw * 10n],
      })
      return
    }
  ...
  const loading = approving || buying || confirming
  const label = loading
    ? approving ? 'Approving…' : confirming ? 'Confirming…' : 'Buying…'
    : needsApproval
    ? 'Approve USDC'
    : 'Buy Coverage'
...
function PremiumCalculator({ product }: { product: Product }) {
  ...
  const rawPremium = (product.premiumRateBps * parsedAmount) / 10000
  ...
  return (
    <div className="space-y-3">
      <div>
        <label className="text-xs text-slate-500 mb-1.5 block">Coverage Amount (USD)</label>
        ...
        {parsedAmount > product.maxPerPositionUsd && (
          <p className="text-red-400 text-xs mt-1">Max {formatUsd(product.maxPerPositionUsd)}</p>
        )}
      </div>
      ...
      {parsedAmount > 0 && (
        <div className="bg-slate-800/60 rounded-lg p-3 space-y-1.5 text-sm">
          <div className="flex justify-between">
            ...
            <span className="text-white font-medium">${premium.toFixed(2)} USDC</span>
          </div>
          <div className="flex justify-between">
            ...
            <span className={cn(
              ...
              multiplier === 1.5 ? 'text-amber-400' :
              multiplier === 2 ? 'text-orange-400' : 'text-red-400',
            )}>
              {multiplier}x
            </span>
          </div>
      ...
      <BuyCoverageButton
        ...
        disabled={!isConnected || !CORE_ADDRESS}
      />

      {!CORE_ADDRESS && (
        <p className="text-xs text-slate-600 text-center">Contract not deployed yet</p>
      )}
    </div>
  )
}
    ...
    <div className="max-w-7xl mx-auto px-6 py-12 min-h-screen">
      <div className="mb-10">
        ...
        <p className="text-slate-400 text-lg max-w-xl">
          Parametric protection against depeg events and rug pulls. Pay a premium, get covered — instantly.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {filteredProducts.map((product) => {
          const utilPct = Math.round((product.totalCommittedUsd / product.poolLimitUsd) * 100)
          return (
            <div
              key={product.id}
            ...
            >
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-slate-800/60 rounded-lg p-3">
                  ...
                  <p className="text-white font-semibold text-sm">{formatUsd(product.maxPerPositionUsd)}</p>
                </div>
                <div className="bg-slate-800/60 rounded-lg p-3">
                  <p className="text-slate-500 text-xs mb-1">Premium Rate</p>
                  <p className="text-white font-semibold text-sm">{(product.premiumRateBps / 100).toFixed(2)}%</p>
                </div>
                <div className="bg-slate-800/60 rounded-lg p-3">
                  <p className="text-slate-500 text-xs mb-1">Duration</p>
                  <p className="text-white font-semibold text-sm">
                    {product.durationHours ? `${product.durationHours}h` : 'Open-ended'}
                  </p>
                </div>
              </div>
              ...
              <div>
                ...
                <p className="text-slate-600 text-xs mt-1">
                  {formatUsd(product.totalCommittedUsd)} committed of {formatUsd(product.poolLimitUsd)} limit
                </p>
              </div>
        ...
        {filteredProducts.length === 0 && (
          <div className="col-span-2 text-center py-16 text-slate-500">
            No {activeTab === 'DEPEG' ? 'depeg' : 'rug pull'} products available.
          </div>
        )}
      </div>
    </div>
